...
from spark_pipeline.g1_safe_teleop.g1_safe_teleop_real_pipeline_config import G1SafeTeleopRealPipelineConfig 
...
class G1SafeTeleopPipeline(BasePipeline[G1SafeTeleopRealPipelineConfig]):
    ...
    def post_physics_step(self, agent_feedback, task_info, action_info):
        ...
        # publish robot state
        self.env.task.pub_robot_state(action_info)
    ...
    def run(self, save_path = None):
        ...
        self.loop_time = self.cfg.env.agent.dt
        while self.pipeline_step < self.max_num_steps or self.max_num_steps < 0:
            ...
            # print(f"Time taken for one step: {end_time - start_t}")
            step_t = time.time() - start_t
            if  step_t < self.cfg.env.agent.dt * self.cfg.env.agent.control_decimation:
                time.sleep(self.cfg.env.agent.dt * self.cfg.env.agent.control_decimation - step_t)
    ...
    def process_and_save_data(self):
        ...
        self.goal_left_frame_base = self.goal_teleop.get("left", None) if self.goal_teleop is not None else None
        self.goal_right_frame_base = self.goal_teleop.get("right", None) if self.goal_teleop is not None else None
        ...
        if self.goal_left_frame_base is not None and self.goal_right_frame_base is not None:
            self.goal_left_frame_world = self.robot_base_frame @ self.goal_left_frame_base
            self.goal_right_frame_world = self.robot_base_frame @ self.goal_right_frame_base
            if hasattr(self.robot_cfg.Frames, "L_ee"):
                self.dist_goal_left = np.linalg.norm(self.robot_frames_world[self.robot_cfg.Frames.L_ee,:3,3] - self.goal_left_frame_world[:3,3])
            else:
                self.dist_goal_left = 0.0
            if hasattr(self.robot_cfg.Frames, "R_ee"):
                self.dist_goal_right = np.linalg.norm(self.robot_frames_world[self.robot_cfg.Frames.R_ee,:3,3] - self.goal_right_frame_world[:3,3])
            else:
                self.dist_goal_right = 0.0
        else:
            self.goal_left_frame_world = None
            self.goal_right_frame_world = None
            self.dist_goal_left = None
            self.dist_goal_right = None
        
        if self.cfg.metric_selection.dist_goal_arm:
            if self.dist_goal_left is not None and self.dist_goal_right is not None:
