...
class UnitreeG1WBTSafePolicy(Policy):
    """Cascade wrapper for Unitree G1 WBT.

    The pre-WBT safety stage uses a sport-mode abstraction: 17 upper-body
    controls plus named virtual base controls. The abstraction also exposes
    ...
    def __init__(
        ...
        **kwargs,
        ...
        wbt_arm_robot_cfg_cfg = getattr(self.cfg, "wbt_arm_robot_cfg", None)
        self.wbt_arm_robot_cfg = (
            initialize_class(wbt_arm_robot_cfg_cfg)
            if wbt_arm_robot_cfg_cfg is not None
            else robot_cfg
        )
        if self.wbt_arm_robot_cfg is None:
            self.wbt_arm_robot_cfg = robot_cfg

        wbt_arm_kinematics_cfg = getattr(self.cfg, "wbt_arm_kinematics", None)
        self.wbt_arm_kinematics = (
            initialize_class(wbt_arm_kinematics_cfg, robot_cfg=self.wbt_arm_robot_cfg)
            if wbt_arm_kinematics_cfg is not None
            else robot_kinematics
        )
        if self.wbt_arm_kinematics is None:
            self.wbt_arm_kinematics = robot_kinematics
        ...
        self.executor = initialize_class(
            self.cfg.executor,
            robot_cfg=robot_cfg,
            robot_kinematics=robot_kinematics,
            arm_robot_cfg=self.wbt_arm_robot_cfg,
            arm_kinematics=self.wbt_arm_kinematics,
        )
        ...
        self.pre_safe_height_priority = bool(getattr(self.cfg, "pre_safe_height_priority", True))
        self.pre_safe_height_priority_deadband = float(
            getattr(self.cfg, "pre_safe_height_priority_deadband", 0.01)
        )
        self.pre_safe_height_priority_xy_deadband = float(
            getattr(self.cfg, "pre_safe_height_priority_xy_deadband", 0.02)
        )
        self.pre_safe_height_priority_yaw_deadband = float(
            getattr(self.cfg, "pre_safe_height_priority_yaw_deadband", 0.05)
        )
        ...
        self._last_wbt_loco_command = None
    ...
    def _planar_yaw_command_from_safe_control(
        ...
        # TeleopPIDPolicy has already applied the inverse mobile-base
        # dynamics map, so its virtual-base controls are body-frame values.
        # LeggedLocomotionTrackingPolicy retains the established world-frame
        # abstraction and still needs the conversion below.
    ...
    def _height_command_from_safe_control(self, control: np.ndarray) -> float:
        ...
        command = float(
            np.clip(
                command,
                -self.pre_safe_height_command_limit,
                self.pre_safe_height_command_limit,
            )
        )
        return command

    def _wbt_height_target_from_command(
        ...
        target_height = current_height + height_command * self.pre_safe_height_target_lookahead
        target_height = float(
    ...
    def _nominal_wbt_command(
        self,
        sport_u_ref: np.ndarray,
        agent_feedback: dict,
        task_info: dict,
    ) -> np.ndarray:
        if not self._base_goal_enabled(task_info):
            zero_fn = getattr(self.executor, "_zero_base_command", None)
            if callable(zero_fn):
                command = np.asarray(zero_fn(), dtype=float).reshape(-1)
            else:
                command = np.zeros(5, dtype=float)
        else:
            # The tracker is the sole nominal base-goal controller.  This
            # keeps WBT and SONIC on the same tracker -> safety -> executor
            # composition and avoids an independent WBT PID command.
            command = np.concatenate(
                [
                    self._planar_yaw_command_from_safe_control(sport_u_ref, agent_feedback),
                    np.zeros(2, dtype=float),
                ]
            )
        if command.shape[0] == 3:
            command = np.concatenate([command, np.zeros(2, dtype=float)])
        if command.shape[0] != 5:
            raise ValueError(f"WBT command must have 3 or 5 entries; got {command.shape[0]}")
        return command
    ...
    def _smooth_wbt_loco_command(
        ...
        if not safe_active:
            self._last_wbt_loco_command = nominal_loco_command.copy()
        ...
        if self._last_wbt_loco_command is None:
            self._last_wbt_loco_command = nominal_loco_command.copy()

        delta = np.clip(
            target - self._last_wbt_loco_command,
            -self.pre_safe_loco_command_rate_limit,
            self.pre_safe_loco_command_rate_limit,
        )
        smoothed = self._last_wbt_loco_command + delta
        self._last_wbt_loco_command = smoothed.copy()
        return smoothed
    ...
    def visualization_context(
        ...
        # The reduced mobile-base safety model is only meaningful while an
        # environment constraint exists.  Using it in the obstacle-free
        # bypass benchmark replaced the whole-body collision visualization
        # with a small head/end-effector subset in MuJoCo.
        if not self._has_obstacles(task_info):
            return None
        if (
        ...
        # Distance-info represents signed surface clearance as -normal*d_min.
        # Visualization requires the scalar d_min matrix; passing the vector
        # tensor made collision coloring silently skip composed policies.
    ...
    def _upper_body_delta_from_sport_control(
        ...
        # Only modify the branch whose collision volume activated the safety
        # constraint.  Applying a torso/left-arm correction to both arms and
        # the waist perturbs the learned balance target and is particularly
        # destabilizing for a rear-torso obstacle in PhysX.
        ...
        # to the normal WBT IK target through the mask below.
    ...
    def act(self, agent_feedback, task_info):
        ...
        nominal_wbt_command = self._nominal_wbt_command(sport_u_ref, agent_feedback, task_info)
        safe_wbt_command = nominal_wbt_command.copy()
        pre_safe_active = bool(safe_control_info.get("trigger_safe", False) or pre_safe_hold_active)
        goal_arrived = bool(
            sport_action_info.get("locomotion_phase") == "arrived" and not pre_safe_active
        )
        ...
        pre_safe_height_priority_active = False
        if pre_safe_loco_applied:
            safe_height_command = self._height_command_from_safe_control(sport_u_safe)
            safe_planar_xy_norm = float(np.linalg.norm(safe_planar_yaw_command[:2]))
            safe_planar_yaw_abs = float(abs(safe_planar_yaw_command[2]))
            pre_safe_height_priority_active = bool(
                self.pre_safe_height_priority
                and abs(safe_height_command) > self.pre_safe_height_priority_deadband
                and safe_planar_xy_norm <= self.pre_safe_height_priority_xy_deadband
                and safe_planar_yaw_abs <= self.pre_safe_height_priority_yaw_deadband
            )
            if pre_safe_height_priority_active:
                safe_wbt_command[:3] = np.zeros(3, dtype=float)
                self._last_wbt_loco_command = np.zeros(3, dtype=float)
            else:
                safe_wbt_command[:3] = self._smooth_wbt_loco_command(
                    nominal_wbt_command[:3],
                    safe_planar_yaw_command,
                    True,
                )
            safe_wbt_command[3] = safe_height_command
        else:
            if goal_arrived:
                # Arrival is a discrete controller-mode event. Rate-limiting
                # the final nonzero gait command keeps the locomotion RNN
                # stepping after the tracker has already declared success.
                safe_wbt_command[:3] = 0.0
                self._last_wbt_loco_command = np.zeros(3, dtype=float)
            else:
                safe_wbt_command[:3] = self._smooth_wbt_loco_command(
                    nominal_wbt_command[:3],
            ...
            if base_goal_enabled:
                safe_height_command = float(nominal_wbt_command[3])
            else:
                safe_height_command = self._height_recovery_command(agent_feedback, task_info)
            safe_wbt_command[3] = safe_height_command
        pre_safe_height_target = self._wbt_height_target_from_command(
        ...
        wbt_task_info = {
            **task_info,
            "wbt_command": safe_wbt_command.copy(),
            "wbt_command_nominal": nominal_wbt_command.copy(),
            "sport_u_ref": sport_u_ref.copy(),
            "sport_u_safe": sport_u_safe.copy(),
            "wbt_loco_safety_override": bool(pre_safe_loco_applied),
            "wbt_goal_arrived": goal_arrived,
        }
        if pre_safe_height_target is not None:
            wbt_task_info["wbt_height"] = pre_safe_height_target
        if upper_body_target_override is not None:
            wbt_task_info["upper_body_target_override"] = upper_body_target_override
            wbt_task_info["upper_body_target_override_mask"] = upper_body_target_override_mask
        if upper_body_target_delta is not None:
            wbt_task_info["upper_body_target_delta"] = upper_body_target_delta
        wbt_u_ref, wbt_action_info = self.executor.act(
            agent_feedback=agent_feedback,
            task_info=wbt_task_info,
        )
        ...
        action_info = {
            ...
            **pre_safe_control_info,
            ...
            "wbt_command_nominal": nominal_wbt_command.copy(),
            "wbt_command_safe": safe_wbt_command.copy(),
            "wbt_loco_command_raw_safe": safe_planar_yaw_command.copy(),
            ...
            "pre_safe_height_priority_active": bool(pre_safe_height_priority_active),
            "pre_safe_height_priority_xy_norm": float(np.linalg.norm(safe_planar_yaw_command[:2])),
            "pre_safe_height_priority_yaw_abs": float(abs(safe_planar_yaw_command[2])),
            ...
            "u_ref": np.asarray(wbt_u_ref, dtype=float).copy(),
            "u_safe": np.asarray(wbt_u_ref, dtype=float).copy(),
        }
        self._add_gripper_info(task_info, action_info)
        return wbt_u_ref, action_info
