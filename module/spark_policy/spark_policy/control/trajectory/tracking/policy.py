...
class TrajTrackingPolicy(BasePolicy):
            ...
            for t in range(traj_len):
                if "left" not in goal_teleop:
                    dof_pos_target, _ = self.robot_kinematics.inverse_kinematics(
                        [target_traj_cur["right"][t]], dof_pos_tmp
                    )
                else:
                    dof_pos_target, _ = self.robot_kinematics.inverse_kinematics(
                        [target_traj_cur["right"][t], target_traj_cur["left"][t]], dof_pos_tmp
                    )
    ...
    def act(self, agent_feedback: dict, task_info: dict):
        ...
        target_traj_cur = {}
        target_traj_cur["right"] = (
            np.linalg.inv(robot_base_frame) @ task_info["goal_teleop"]["right"]
        )
        if "left" in task_info["goal_teleop"]:
            target_traj_cur["left"] = (
                np.linalg.inv(robot_base_frame) @ task_info["goal_teleop"]["left"]
            )
