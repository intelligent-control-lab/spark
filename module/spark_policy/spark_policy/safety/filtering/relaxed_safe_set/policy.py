...
from spark_policy.safety.filtering.basic_safe_set import BasicSafeSetAlgorithm


class RelaxedSafeSetAlgorithm(BasicSafeSetAlgorithm):
    """
    Relaxed-SSA

    SSA with multiple phi_dot <= -eta_ssa constraints and slack variables for each constraint
    ...
    def __init__(self, **kwargs):
        ...
        # default values
        eta_ssa = kwargs.get("eta_ssa", 0.3)
        self.eta_default = float(0.3 if eta_ssa is None else eta_ssa)
        self.control_weight = np.array(kwargs["control_weight"])
        slack_weight = kwargs.get("slack_weight", 1e3)
        self.slack_weight_default = float(1e3 if slack_weight is None else slack_weight)
    ...
    def qp_solver(self, u_ref, Q_u, Lg, Lf, eps_=1.00e-2, abs_=1.00e-2):
        ...
        if self.slack_regularization_order == 1:
            ...
            """
            Q_s = sparse.diags(
                self.slack_weight_default * np.ones(self.safety_index.num_constraint)
            )
            q_s = np.zeros(m)
        else:
            raise ValueError(
                f"slack_regularization_order {self.slack_regularization_order} not supported, must be 1 or 2"
            )
    ...
    def safe_control(
        ...
        index_seconds = time.perf_counter() - timing_start
        ...
        # jacobian_left = self.robot_kinematics.get_jacobian(self.robot_cfg.Frames.L_ee.name)[:3, :]
        # jacobian_right = self.robot_kinematics.get_jacobian(self.robot_cfg.Frames.R_ee.name)[:3, :]
        ...
        if self.has_active_violation(phi):
            ...
            solve_seconds = time.perf_counter() - timing_start
        else:
            trigger_safe = False
            u_safe = u_ref
        ...
        info = {
            ...
            "safety_index_seconds": index_seconds,
            "safety_solve_seconds": solve_seconds,
            "safety_constraint_count": int(self.safety_index.num_constraint),
        }
        if self.safety_index.environment_point_indices is not None:
            info["safety_obstacle_indices"] = self.safety_index.environment_point_indices.copy()
