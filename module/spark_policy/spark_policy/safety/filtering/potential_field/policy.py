...
class BasicPotentialFieldMethod(ValueBasedSafeAlgorithm):
    ...
    def safe_control(
        ...
        phi_env = np.asarray(phi[: self.safety_index.num_constraint_env], dtype=float)
        env_mask = np.asarray(self.safety_index.env_collision_mask, dtype=bool).reshape(-1)
        active_env_id = np.flatnonzero(env_mask & (phi_env > 0.0))

        if active_env_id.size:
            trigger_safe = True
            max_phi_idx = active_env_id[np.argmax(phi_env[active_env_id])]
            ...
            Cartesian_Lg = np.einsum(
                "ij,ijk->ik",
                Lg,
                np.linalg.pinv(jacobian_full @ s_matrix @ self.robot_cfg.dynamics_g(x)),
            )
            ...
            u_safe = (
                np.linalg.pinv(critical_jacobian @ s_matrix @ self.robot_cfg.dynamics_g(x))
                @ u_safe_cartesian
            )
        else:
            trigger_safe = False
