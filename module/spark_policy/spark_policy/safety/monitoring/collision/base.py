...
class BasicCollisionSafetyIndex(BaseSafetyIndex):
    def __init__(self, robot_kinematics: RobotKinematics, **kwargs):
        ...
        self.collision_vol_frame_ids = list(
            self.robot_cfg.CollisionVol.keys()
        )  # frame ids, to index the frames computed by FK
        self.collision_vol_geom = [
            self.robot_cfg.CollisionVol[fid] for fid in self.collision_vol_frame_ids
        ...
        self.nearest_points_per_link = int(kwargs.get("nearest_points_per_link", 0))
        self.environment_point_indices = None

        self.num_constraint_self = (
            self.num_robot_vol * (self.num_robot_vol - 1) // 2 if self.enable_self_collision else 0
        )

        # create mask for self collision check
        self.self_collision_mask = self.enable_self_collision * np.ones(
            (self.num_robot_vol, self.num_robot_vol), dtype=bool
        )
        ...
        for frame_i in self.robot_cfg.SelfCollisionVolIgnored:
            ...
            self.self_collision_mask[:, i] = False
        self.min_dist_self = self.min_distance["self"] * np.ones(
            (self.num_robot_vol, self.num_robot_vol)
        )
    ...
    def update_obstacle_info(self, task_info=None):
        ...
        # min dist matrix
        self.min_dist_env = self.min_distance["environment"] * np.ones(
            (self.num_robot_vol, self.num_obstacle_vol)
        )
    ...
    def get_vol_info(self, x, task_info):
        ...
        assert "frames_world" in task_info["obstacle"], (
            "frames_world not found in task_info['obstacle']"
        )
        ...
        robot_frames = self.robot_kinematics.forward_kinematics(
            self.robot_cfg.decompose_state_to_dof_pos(x)
        )
        trans_world2base = self.robot_kinematics.update_base_frame(
            trans_world2base, self.robot_cfg.decompose_state_to_dof_pos(x)
        )
        # get collision vol frames
        robot_vol_frames = np.stack(
            [robot_frames[fid] for fid in self.collision_vol_frame_ids], axis=0
        )  # [num_robot_vol, 4, 4]
        # transform to world frame
        robot_vol_frames_world = np.einsum("ij,njk->nik", trans_world2base, robot_vol_frames)
        self.environment_point_indices = None
        if (
            self.nearest_points_per_link > 0
            and len(obstacle_vol_frames) > self.nearest_points_per_link
        ):
            from scipy.spatial import cKDTree

            obstacle_positions = np.asarray(obstacle_vol_frames)[:, :3, 3]
            robot_positions = robot_vol_frames_world[:, :3, 3]
            count = min(self.nearest_points_per_link, len(obstacle_positions))
            _, nearest = cKDTree(obstacle_positions).query(robot_positions, k=count)
            selected = np.unique(np.asarray(nearest, dtype=int).reshape(-1))
            self.environment_point_indices = selected
            obstacle_vol_frames = np.asarray(obstacle_vol_frames)[selected]
            obstacle_vol_geom = np.asarray(obstacle_vol_geom, dtype=object)[selected]
            obstacle_vol_vel = np.asarray(obstacle_vol_vel)[selected]

        selected_task_info = {"obstacle": {"num": len(obstacle_vol_frames)}}
        self.update_obstacle_info(selected_task_info)
        ...
        for index, frame_id in enumerate(self.collision_vol_frame_ids):
            jacobian = self.robot_kinematics.get_jacobian(frame_id.name)
            robot_collision_vol.append(
                Volume(
                    frame=robot_vol_frames_world[index],
                    geometry=self.collision_vol_geom[index],
    ...
    def compute_pairwise_info(self, volume_list_1, volume_list_2):
        ...
        """
        count_1 = len(volume_list_1)
        count_2 = len(volume_list_2)
        if count_1 == 0 or count_2 == 0:
            return (
                np.zeros((count_1, count_2, 3), dtype=float),
                np.zeros((count_1, count_2, 3), dtype=float),
                np.zeros((count_1, count_2, 3), dtype=float),
                np.zeros((count_1, count_2, 3, 3), dtype=float),
            )
    ...
    def compute_pairwise_vol_info(self, robot_collision_vol, obstacle_collision_vol):
        d_env, v_env, normal_env, curv_env = self.compute_pairwise_info(
            robot_collision_vol, obstacle_collision_vol
        )
        if self.num_constraint_self > 0:
            d_self, v_self, normal_self, curv_self = self.compute_pairwise_info(
        ...
        if self.num_constraint_self > 0:
            ...
            normal[-self.num_constraint_self :] = normal_self[
                np.triu_indices(normal_self.shape[0], k=1)
            ]
    ...
    def phi_cartesian(
        self, robot_frame_closest: np.ndarray, task_info: dict, trans_world2base: np.ndarray
    ):
        """
    ...
    def decode_constraint_info(self, vec, name):
        ...
        assert vec.shape[0] == self.num_constraint, (
            f"vec shape {vec.shape} does not match num_constraint {self.num_constraint}"
        )
    ...
    def jacobian_full(self, dof_pos):
        ...
        jacobian = np.array(
            [
                self.robot_kinematics.get_jacobian(frame_id.name)[:3, :]
                for frame_id in self.collision_vol_frame_ids
            ]
        )
        ...
        jacobian_full_env = np.zeros(
            (self.num_robot_vol, self.num_obstacle_vol, 3, jacobian.shape[2])
        )
        jacobian_full_env[:, :, :, :] = jacobian[:, np.newaxis, :, :]
        jacobian_full = np.zeros((self.num_constraint, 3, jacobian.shape[2]))
        jacobian_full[: self.num_constraint_env, :, :] = jacobian_full_env.reshape(
            -1, 3, jacobian.shape[2]
        )

        if self.num_constraint_self > 0:
            jacobian_full_self = np.zeros(
                (self.num_robot_vol, self.num_robot_vol, 3, jacobian.shape[2])
            )
            jacobian_full_self[:, :, :, :] = jacobian[:, np.newaxis, :, :]
            jacobian_full[self.num_constraint_env :, :, :] = jacobian_full_self[
                np.triu_indices(self.num_robot_vol, k=1)
            ].reshape(-1, 3, jacobian.shape[2])
    ...
    def jacobian_dot_full(self, dof_pos, dof_vel):
        ...
        jacobian_dot = np.array(
            [
                self.robot_kinematics.get_jacobian_dot(frame_id.name)[:3, :]
                for frame_id in self.collision_vol_frame_ids
            ]
        )
        # for frame in self.robot_cfg.Frames:
        ...
        jacobian_dot_full_env = np.zeros(
            (self.num_robot_vol, self.num_obstacle_vol, 3, jacobian_dot.shape[2])
        )
        jacobian_dot_full_env[:, :, :, :] = jacobian_dot[:, np.newaxis, :, :]
        jacobian_dot_full = np.zeros((self.num_constraint, 3, jacobian_dot.shape[2]))
        jacobian_dot_full[: self.num_constraint_env, :, :] = jacobian_dot_full_env.reshape(
            -1, 3, jacobian_dot.shape[2]
        )

        if self.num_constraint_self > 0:
            jacobian_dot_full_self = np.zeros(
                (self.num_robot_vol, self.num_robot_vol, 3, jacobian_dot.shape[2])
            )
            jacobian_dot_full_self[:, :, :, :] = jacobian_dot[:, np.newaxis, :, :]
            jacobian_dot_full[self.num_constraint_env :, :, :] = jacobian_dot_full_self[
                np.triu_indices(self.num_robot_vol, k=1)
            ].reshape(-1, 3, jacobian_dot.shape[2])
