...
class SecondOrderCollisionSafetyIndex(BasicCollisionSafetyIndex):
    def __init__(self, robot_kinematics: RobotKinematics, **kwargs):
        ...
        assert self.robot_cfg.dynamics_order == 2, (
            "SecondOrderCollisionSafetyIndex requires second-order robot dynamics"
        )
        ...
        self._phi_dot = lambda d, v, a, normal, curv: (
            -self.n
            * (np.sum(-d * normal, axis=1) + eps) ** (self.n - 1)
    ...
    def phi(self, x: np.ndarray, task_info: dict):
        ...
        s_matrix = np.hstack(
            [np.zeros((len(x) - self.num_dof, self.num_dof)), np.eye(len(x) - self.num_dof)]
