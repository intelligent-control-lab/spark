...
class FirstOrderCollisionSafetyIndexApprox(BasicCollisionSafetyIndex):
    def __init__(self, robot_kinematics: RobotKinematics, **kwargs):
        ...
        assert self.robot_cfg.dynamics_order == 1, (
            "FirstOrderCollisionSafetyIndexApprox requires first-order robot dynamics"
        )
    ...
    def construct_vol_list(
        ...
        for _x in x_list:
            ...
            for _dof_pos in [dof_pos, dof_pos_forward, dof_pos_backward]:
                ...
                trans_world2base = self.robot_kinematics.update_base_frame(
                    trans_world2base, _dof_pos
                )
                # # get collision vol frames
                collision_vol_frames_base = np.stack(
                    [frames[fid] for fid in self.collision_vol_frame_ids], axis=0
                )  # [num_collision_vol, 4, 4]
        ...
        return (
            np.array(vol_frame_list_robot),
            np.array(self.collision_vol_geom),
            np.tile(obstacle_vol_frames_world, (B, 1, 1, 1)),
            np.array(obstacle_vol_geom),
        )
    ...
    def phi(
        ...
        vol_frame_list_robot, vol_geom_list_robot, obstacle_vol_frames_world, obstacle_vol_geom = (
            self.construct_vol_list(x, task_info)
        )
        ...
        if self.num_constraint_self > 0:
            ...
            dist[:, -self.num_constraint_self :] = dist_self_curr_dof[
                :, triu_indices[0], triu_indices[1]
            ]
            dist_forward_dof[:, -self.num_constraint_self :] = dist_self_forward_dof[
                :, triu_indices[0], triu_indices[1]
            ]
            dist_backward_dof[:, -self.num_constraint_self :] = dist_self_backward_dof[
                :, triu_indices[0], triu_indices[1]
            ]
