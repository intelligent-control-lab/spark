...
from spark_policy.safety import SPARK_SAFETY_ROOT
...
class SecondOrderNNCollisionSafetyIndex(BasicCollisionSafetyIndex):
    def __init__(self, robot_kinematics: RobotKinematics, **kwargs):
        ...
        assert self.robot_cfg.dynamics_order == 2, (
            "SecondOrderNNCollisionSafetyIndex requires second-order robot dynamics"
        )

        self.n = kwargs["phi_n"]
        self.k = kwargs["phi_k"]

        self.phi_nn_path = os.path.join(
            SPARK_SAFETY_ROOT,
            "monitoring/collision/resources/onnx_model",
            kwargs["phi_nn_path"],
        )
        ...
        self._phi = lambda d, v, normal, nn_term: (
            self.dmin - np.sum(-d * normal, axis=1) + nn_term + self.k * np.sum(v * normal, axis=1)
        )

        # General implementation of the time derivative of the second order safety index in the Cartesian space
        self._phi_dot = lambda d, v, a, normal, curv, nn_term_grad: (
            np.sum(v * normal, axis=1)
            - nn_term_grad.reshape(-1) * np.sum(v * normal, axis=1)
            + self.k * np.sum(a * normal, axis=1)
            - self.k * np.sum(v * (curv @ v[..., None]).squeeze(-1), axis=1)
        )

        self.Cartesian_Lg = lambda normal: self.k * normal
        self.Cartesian_Lf = lambda d, v, normal, curv, nn_term_grad: (
            np.sum(v * normal, axis=1)
            - nn_term_grad.reshape(-1) * np.sum(v * normal, axis=1)
            - self.k * np.sum(v * (curv @ v[..., None]).squeeze(-1), axis=1)
        )
    ...
    def phi(self, x: np.ndarray, task_info: dict):
        ...
        torch_input = torch.from_numpy(
            np.sum(d * normal, axis=1, dtype=np.float32).reshape(-1, 1)
        ).requires_grad_(True)
        ...
        s_matrix = np.hstack(
            [np.zeros((self.num_dof, self.num_dof)), np.eye(self.num_dof)]
        )  # only take the velocity part

        Lg = np.einsum("ij,ijk->ik", Cartesian_Lg, jacobian_full @ s_matrix @ gx)
        Lf = (
            Cartesian_Lf
            + np.einsum("ij,ijk->ik", Cartesian_Lg, jacobian_dot_full) @ dof_vel
            + np.einsum("ij,ijk->ik", Cartesian_Lg, jacobian_full) @ s_matrix @ fx
        )
