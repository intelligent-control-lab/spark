...
from spark_robot import KukaIIWA14DualArmDynamic1Config, KukaIIWA14DualArmKinematics
import numpy as np
...
class RRTConnect:
    ...
    def plan(self, start, goal, cur_full_q, planning_joint_list):
        ...
        if not self.is_collision_free(self.start_node) or not self.is_collision_free(
            self.goal_node
        ):
            return [], False
    ...
    def is_collision_free(self, node):
        ...
        pin.computeCollisions(
            self.model, self.data, self.collision_model, self.collision_data, full_robot_q, True
        )
...
class RRTConnectPolicy(BasePolicy):
    def __init__(self, robot_cfg: RobotConfig, robot_kinematics: RobotKinematics) -> None:
        ...
        self.model, self.collision_model, self.visual_model = (
            self.robot_kinematics.model,
            self.robot_kinematics.collision_model,
            self.robot_kinematics.visual_model,
        )
...
if __name__ == "__main__":
    ...
    robot_kinematics = KukaIIWA14DualArmKinematics(robot_cfg)
    rrt_policy = RRTConnectPolicy(robot_cfg, robot_kinematics)
    cur_config = np.array([q for _, q in robot_cfg.DefaultDoFVal.items()])

    right_cart = np.array(
        [[1.0, 0, 0.0, 0.5], [0.0, 1.0, 0.0, -0.6], [-0.0, 0.0, 1.0, 0.2], [0.0, 0.0, 0.0, 1.0]]
    )
    left_cart = np.array(
        [[1.0, 0, 0.0, 0.7], [0.0, 1.0, 0.0, 0.3], [-0.0, 0.0, 1.0, 0.5], [0.0, 0.0, 0.0, 1.0]]
    )

    dof_pos_target, _ = robot_kinematics.inverse_kinematics(
        [left_cart, right_cart], current_lr_arm_motor_q=cur_config
    )
    ...
    plan_arm_joints = [
        model.names[idx] for idx in range(1, model.njoints) if model.joints[idx].nq > 0
    ]
    task_info = {}
    agent_feedback = {}
