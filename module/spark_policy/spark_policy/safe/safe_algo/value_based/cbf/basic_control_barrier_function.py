...
from spark_policy.safe.safe_algo.value_based.base.value_based_safe_algo import ValueBasedSafeAlgorithm

class BasicControlBarrierFunction(ValueBasedSafeAlgorithm):
    ...
    def lambda_fn(self, phi):
        lambda_values = np.where(
                        (self.safety_index.phi_mask.reshape(-1,1) > 0),
    ...
    def qp_solver(self,
        ...
        :param Lf: Constraint violation (CBF condition).
        """
        n = u_ref.shape[0]
        m = Lf.shape[0]
