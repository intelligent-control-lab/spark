...
class VizColor:
    ...
    # Collision volumes should remain unobtrusive at normal operating
    # distances. The pipeline interpolates from ``far`` to ``near`` as the
    # signed surface clearance decreases, then reserves red for contact.
    # Keep a faint non-zero overlay even when no obstacle is present. The
    # near-white far color avoids a dark blue cast where sparse sphere covers
    # overlap in Isaac, while the closer state remains visible but translucent.
    collision_volume_far = [0.90, 0.96, 1.0, 0.025]
    collision_volume_near = [0.25, 0.58, 1.0, 0.12]
    collision_volume = collision_volume_far
    collision_volume_ignored = [0.1, 0.1, 0.1, 0.0]
    # Task and keyboard/debug obstacles deliberately share one palette across
    # MuJoCo and Isaac. Safety state may temporarily override this with red.
    obstacle_orange = [0.90, 0.24, 0.03, 0.90]
    obstacle_debug = obstacle_orange
    obstacle_task = obstacle_orange
...
COLLISION_CONTACT_TOLERANCE = 1.0e-3
COLLISION_COLOR_FAR_DISTANCE = 0.30


def collision_volume_distance_color(
    distance,
    minimum_distance=0.05,
    *,
    far_distance=COLLISION_COLOR_FAR_DISTANCE,
    ignored=False,
    opacity_scale=1.0,
    opacity_floor=0.0,
):
    """Map signed surface clearance to a simulator-neutral RGBA color."""
    opacity_scale = float(opacity_scale)
    if not np.isfinite(opacity_scale) or opacity_scale < 0.0:
        raise ValueError("opacity_scale must be finite and non-negative")
    opacity_floor = float(opacity_floor)
    if not np.isfinite(opacity_floor) or not 0.0 <= opacity_floor <= 1.0:
        raise ValueError("opacity_floor must be between zero and one")

    def scaled(color):
        color = np.asarray(color, dtype=np.float64).copy()
        color[3] = np.clip(color[3] * opacity_scale, 0.0, 1.0)
        if color[3] > 0.0:
            color[3] = max(color[3], opacity_floor)
        return color

    if ignored:
        color = scaled(VizColor.collision_volume_far)
        color[3] *= 0.25
        return color

    distance = float(distance)
    if np.isfinite(distance) and distance <= COLLISION_CONTACT_TOLERANCE:
        return scaled(VizColor.collision)

    minimum_distance = max(float(minimum_distance), COLLISION_CONTACT_TOLERANCE)
    far_distance = max(float(far_distance), minimum_distance + 1.0e-6)
    if not np.isfinite(distance):
        proximity = 0.0
    else:
        proximity = float(
            np.clip((far_distance - distance) / (far_distance - minimum_distance), 0.0, 1.0)
        )
        proximity = proximity * proximity * (3.0 - 2.0 * proximity)
    far = np.asarray(VizColor.collision_volume_far, dtype=np.float64)
    near = np.asarray(VizColor.collision_volume_near, dtype=np.float64)
    return scaled((1.0 - proximity) * far + proximity * near)
...
class Geometry:
    def __init__(self, type, **kwargs):
        ...
        if self.type == "sphere":
            ...
            self.size = np.array(
                [kwargs.get("length", 1.0), kwargs.get("width", 1.0), kwargs.get("height", 1.0)],
                dtype=np.float32,
            )
        else:
...
@njit
def compute_pairwise_distance_info(
    frame_list_1,
    velocity_list_1,
    type_list_1,
    size_list_1,
    ...
    for i in range(N1):
        for j in range(N2):
            ...
            if type1 == 0 and type2 == 0:
                ...
                d, v, n, c = sphere_to_sphere_distance_info(
                    frame1, velocity1, size1, frame2, velocity2, size2
                )
            else:
                # TODO: Sphere-Box
                raise NotImplementedError(
                    "Currently only Sphere-Sphere distance calculation is implemented. Make sure to use approximated safety index si1a or si2a"
                )
...
def sphere_to_sphere_distance_info(frame1, velocity1, size1, frame2, velocity2, size2):
    ...
    if abs(d_center) > 1e-8:  # Avoid division by zero
        n = [
            ...
            dy / d_center,
            dz / d_center,
        ]  # Normalized vector from sphere 1 to sphere 2
        ...
        c = [
            [(1 / d_center) - (outer_product[i][j] / d_center**3) for j in range(3)]
            for i in range(3)
        ]
    else:
...
@njit
def compute_distance_kernel(
    frame_list_1, type_list_1, size_list_1, frame_list_2, type_list_2, size_list_2
):
    # Ensure the input shapes are correct
    assert frame_list_1.shape[0] == frame_list_2.shape[0], (
        "Batch size must be the same for both frame lists"
    )
        ...
        for i in range(N1):
            for j in range(N2):
                ...
                # Sphere-Box
                    ...
                    raise NotImplementedError(
                        "Currently only Sphere-Sphere and Sphere-Box distance calculations are implemented."
                    )
