...
from .isaac_articulation import FIXED_BASE_ISAAC_ARTICULATION, ISAAC_CAPABILITY
from .arm_collision_geometry import whole_body_collision_mappings
from .simulator_dynamics import R1_LITE_SIMULATOR_DYNAMICS


class GalaxeaR1LiteFixedBaseDynamic2CollisionConfig(RobotConfig):
    simulator_dynamics = R1_LITE_SIMULATOR_DYNAMICS
    agent_class_names = {
        "mujoco": "GalaxeaR1LiteFixedBaseAgent",
        "isaac": "GalaxeaR1LiteFixedBaseIsaacAgent",
    }
    ...
    kinematics_class_name = "GalaxeaR1LiteFixedBaseKinematics"
    mujoco_model_path = "galaxea_r1lite/r1lite_fixed_base_scene.xml"
    ...
    ControlLimit = {
        Control.aTorsoJoint1: 1,
        Control.aTorsoJoint2: 1,
        Control.aTorsoJoint3: 1,
        Control.aRightJoint1: 1,
        Control.aRightJoint2: 1,
        Control.aRightJoint3: 1,
        Control.aRightJoint4: 1,
        Control.aRightJoint5: 1,
        Control.aRightJoint6: 1,
        Control.aLeftJoint1: 1,
        Control.aLeftJoint2: 1,
        Control.aLeftJoint3: 1,
        Control.aLeftJoint4: 1,
        Control.aLeftJoint5: 1,
        Control.aLeftJoint6: 1,
    }
    ...
    def dynamics_g(self, state):
        ...
        """
        g_x = np.zeros((2 * self.num_dof, len(self.Control)))
        # DoFs are ordered torso/left/right while acceleration controls retain
        # the hardware-facing torso/right/left order. Map by joint name rather
        # than assuming that the two public enum values are interchangeable.
        for control in self.Control:
            dof = self.DoFs[control.name.removeprefix("a")]
            g_x[self.num_dof + int(dof), int(control)] = 1.0
    ...
    CollisionVolBodyNames, CollisionVolLocalOffsets = whole_body_collision_mappings(Frames)
