...
class GalaxeaR1LiteFixedBaseKinematics(RobotKinematics):
    ...
    def _add_configured_collision_frames(self, model):
        """Attach configured sphere centers to their actual link frames."""
        ...
        for frame in self.robot_cfg.CollisionVol:
            ...
            if body_name == "base_link" and not model.existFrame(body_name):
                # The MJCF root body is named ``robot`` while the URDF/Isaac
                # articulation retains the physical ``base_link`` name.
                body_name = "robot"
...
if __name__ == "__main__":
    ...
    arm_ik = GalaxeaR1LiteFixedBaseKinematics(GalaxeaR1LiteFixedBaseDynamic1Config())
