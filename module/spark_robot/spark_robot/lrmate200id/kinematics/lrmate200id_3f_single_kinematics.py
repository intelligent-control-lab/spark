...
class LRMate200iD3fSingleKinematics(RobotKinematics):
    def __init__(self, robot_cfg: RobotConfig, **kwargs) -> None:
        super().__init__(robot_cfg)
        self.robot_cfg = robot_cfg
        self.mixed_jointsToLockIDs = self.robot_cfg.joint_to_lock
        self.kinematics_model_path = 'lrmate200id/lrmate200id_3f_single.xml'
    ...
    def _init_fixed_base_kinematics(self):
        ...
        self.opti.minimize(10 * self.translational_cost + 5.0*self.rotation_cost + 0.01 * self.regularization_cost + 0.01 * self.smooth_cost)
    ...
    def _init_whole_body_kinematics(self):
        print("Initializing LRMate200iD3fSingleKinematics")
    ...
    def add_extra_frames(self, model):    
        model.addFrame(
            pin.Frame('R_ee',
                     model.getJointId('joint_6'),
                      pin.SE3(np.array([[0, 0, -1],
                                        [0, 1, 0],
                                        [1, 0, 0]]),
                              np.array([0.2,0,0]).T),
    ...
    def update_base_frame(self, trans_world2base, dof):
        try:
            R = trans_world2base[:3, :3]#its first 3*3 matrix is rotaion matrix
        ...
        return trans_world2base #4*4 homogeneous transformation matrix
...
if __name__ == "__main__":
    ...
    arm_ik = LRMate200iD3fSingleKinematics()
