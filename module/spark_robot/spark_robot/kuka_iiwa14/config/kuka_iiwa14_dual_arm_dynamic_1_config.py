...
from .isaac_articulation import KUKA_DUAL_ARM_GRIPPERS


class KukaIIWA14DualArmDynamic1Config(RobotConfig):
    simulator_dynamics = SimulatorDynamicsSpec(physics_dt=0.005, control_decimation=4)
    agent_class_names = {
        "mujoco": "KukaIIWA14DualArmAgent",
        "isaac": "KukaIIWA14DualArmIsaacAgent",
    }
    isaac_articulation = IsaacArticulationSpec(
        urdf_path="kuka_iiwa14/iiwa14.urdf",
        joint_names=tuple(
            f"{side}_iiwa_joint_{index}" for side in ("right", "left") for index in range(1, 8)
        ),
        merge_fixed_joints=True,
        instances=(
            IsaacArticulationInstanceSpec(
                "right", translation=(0.0, -0.3, 0.0), rpy=(0.0, 0.0, -np.pi / 2)
            ),
            IsaacArticulationInstanceSpec(
                "left", translation=(0.0, 0.3, 0.0), rpy=(0.0, 0.0, np.pi / 2)
            ),
        ),
        allow_self_collision=True,
        grippers=KUKA_DUAL_ARM_GRIPPERS,
        stiffness=2000.0,
        damping=160.0,
    )
    backend_capability_overrides = {
        "isaac": BackendCapability(
            ...
            notes="CUDA batched hold/tracking validated dual iiwa articulation.",
        )
    }
    ...
    kinematics_class_name = "KukaIIWA14DualArmKinematics"
    mujoco_model_path = "kuka_iiwa14/iiwa14_2f85_dual.xml"
    # mujoco_model_path = "kuka_iiwa14/iiwa14_dual_scene.xml"
    joint_to_lock = [
    ...
    MujocoMotorKps = {
        ...
        MujocoMotors.RightJoint7: 1000.0,
        ...
        MujocoMotors.LeftJoint7: 1000.0,
    }
