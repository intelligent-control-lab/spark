...
class UnitreeG1WholeBodyDynamic2Config(RobotConfig):
    agent_class_names = {
    ...
    backend_capability_overrides = {
        "isaac": unitree_g1_isaac_capability("UnitreeG1WholeBodyIsaacAgent")
    }
    dynamics_variant = "floating_base_acceleration"
    dynamics_order = 2
    dynamics_is_linear = False
    dynamics_full_state_only = True
    simulator_dynamics = replace(
        UNITREE_G1_SIMULATOR_DYNAMICS,
        command_contract="whole_body_target_or_wbt",
        control_mode="effort_pd",
    ...
    kinematics_class_name = "UnitreeG1WholeBodyKinematics"
    mujoco_model_path = "unitree_g1/mjcf/g1_29dof_whole_body.xml"
    joint_to_lock = []
    ...
    @property
    def num_state(self):
        # Floating-base qpos uses a four-value quaternion while qvel uses a
        # three-value angular velocity, so nq + nv = 36 + 35.
        return int(2 * len(self.DoFs) - 1)

    def compose_state_from_dof(self, dof_pos, dof_vel):
        """
        dof_pos: [num_dof,]
        """
        dof_pos = np.asarray(dof_pos).reshape(-1)
        dof_vel = np.asarray(dof_vel).reshape(-1)
        if dof_vel.size == self.num_dof:
            # Isaac exposes an index-aligned velocity vector with a dummy slot
            # at quaternion-W (qpos index 6). The physical floating-base qvel
            # has only three angular components, so remove that placeholder.
            dof_vel = np.concatenate((dof_vel[:6], dof_vel[7:]))
        if dof_vel.size != self.num_dof - 1:
            raise ValueError(
                f"Whole-body Dynamic2 expects {self.num_dof - 1} physical "
                f"velocities (or {self.num_dof} aligned entries), got {dof_vel.size}"
            )
        state = np.concatenate((dof_pos, dof_vel), axis=0)
        return state
    ...
    def dynamics_g(self, state):
        ...
        # The first six velocity coordinates are the unactuated floating base;
        # the remaining 29 accelerations correspond to Control.
        g_x[self.num_dof + 6 :, :] = np.eye(len(self.Control))
    ...
    CollisionVol = {
        Frames.left_hip_pitch_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.left_hip_roll_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.left_hip_yaw_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.left_knee_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.left_ankle_pitch_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.left_ankle_roll_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.right_hip_pitch_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.right_hip_roll_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.right_hip_yaw_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.right_knee_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.right_ankle_pitch_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.right_ankle_roll_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
