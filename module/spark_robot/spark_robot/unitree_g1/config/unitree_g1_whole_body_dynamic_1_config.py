...
class UnitreeG1WholeBodyDynamic1Config(RobotConfig):
    agent_class_names = {
    ...
    dynamics_variant = "floating_base_velocity"
    dynamics_order = 1
    ...
    def decompose_state_to_dof_vel(self, state):
        ...
        return: [num_dof,]
        """
        # First order dynamic state does not have velocity. The free-flyer
        # quaternion has 4 qpos entries but 3 angular velocity entries.
    ...
    def dynamics_g(self, state):
        ...
        g_x = np.zeros((self.num_state, num_control))
        g_x[7:36, :] = np.eye(num_control)
        return g_x
