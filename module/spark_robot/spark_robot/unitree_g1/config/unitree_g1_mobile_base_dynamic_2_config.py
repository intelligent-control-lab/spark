...
from spark_robot.unitree_g1.config.isaac_capability import unitree_g1_isaac_capability
from spark_robot.unitree_g1.config.simulator_dynamics import UNITREE_G1_SIMULATOR_DYNAMICS
...
class UnitreeG1MobileBaseDynamic2Config(RobotConfig):
    simulator_dynamics = UNITREE_G1_SIMULATOR_DYNAMICS
    agent_class_names = {
        "mujoco": "UnitreeG1MobileBaseMujocoAgent",
        "isaac": "UnitreeG1MobileBaseIsaacAgent",
        "real": "UnitreeG1RealAgent",
    }
    backend_capability_overrides = {
        "isaac": unitree_g1_isaac_capability("UnitreeG1MobileBaseIsaacAgent")
    }
    ...
    kinematics_class_name = "UnitreeG1MobileBaseKinematics"
    mujoco_model_path = "unitree_g1/mjcf/g1_29dof_mobile_base.xml"
    joint_to_lock = []
    ...
    def dynamics_f(self, state):
        ...
        # Update the global linear and rotation term
        RotYaw_global = positions[self.DoFs.RotYaw]
        vLinearX_global = velocities[self.DoFs.LinearX]
        vLinearY_global = velocities[self.DoFs.LinearY]
        vRotYaw_global = velocities[self.DoFs.RotYaw]

        vLinearX_local = vLinearX_global * np.cos(RotYaw_global) + vLinearY_global * np.sin(
            RotYaw_global
        )
        vLinearY_local = -vLinearX_global * np.sin(RotYaw_global) + vLinearY_global * np.cos(
            RotYaw_global
        )
        f_x[self.num_dof + self.DoFs.LinearX] = -vLinearX_local * vRotYaw_global * np.sin(
            RotYaw_global
        ) - vLinearY_local * vRotYaw_global * np.cos(RotYaw_global)
        f_x[self.num_dof + self.DoFs.LinearY] = vLinearX_local * vRotYaw_global * np.cos(
            RotYaw_global
        ) - vLinearY_local * vRotYaw_global * np.sin(RotYaw_global)
    ...
    def dynamics_g(self, state):
        ...
        RotYaw = float(np.asarray(state[self.DoFs.RotYaw]).reshape(-1)[0])

        g_x[self.num_dof + self.DoFs.LinearX, self.Control.aLinearX] = np.cos(RotYaw)
        g_x[self.num_dof + self.DoFs.LinearY, self.Control.aLinearX] = np.sin(RotYaw)
        g_x[self.num_dof + self.DoFs.LinearX, self.Control.aLinearY] = -np.sin(RotYaw)
        g_x[self.num_dof + self.DoFs.LinearY, self.Control.aLinearY] = np.cos(RotYaw)
    ...
    MujocoMotorKps = {
        MujocoMotors.LinearX: 100000.0,
        MujocoMotors.LinearY: 100000.0,
        MujocoMotors.RotYaw: 100000.0,
    ...
    MujocoMotorKds = {
        ...
        MujocoMotors.WaistYaw: 10.0,
        MujocoMotors.WaistRoll: 10.0,
        MujocoMotors.WaistPitch: 10.0,
        MujocoMotors.LeftShoulderPitch: 4.0,
        MujocoMotors.LeftShoulderRoll: 4.0,
        MujocoMotors.LeftShoulderYaw: 4.0,
        MujocoMotors.LeftElbow: 4.0,
        MujocoMotors.LeftWristRoll: 2.0,
        MujocoMotors.LeftWristPitch: 2.0,
        MujocoMotors.LeftWristYaw: 2.0,
        MujocoMotors.RightShoulderPitch: 4.0,
        MujocoMotors.RightShoulderRoll: 4.0,
        MujocoMotors.RightShoulderYaw: 4.0,
        MujocoMotors.RightElbow: 4.0,
        MujocoMotors.RightWristRoll: 2.0,
        MujocoMotors.RightWristPitch: 2.0,
        MujocoMotors.RightWristYaw: 2.0,
    }
    ...
    MujocoMotor_to_Control = {
        MujocoMotors.LinearX: Control.aLinearX,
        MujocoMotors.LinearY: Control.aLinearY,
        MujocoMotors.RotYaw: Control.aRotYaw,
    ...
    RealMotor_to_Control = {
        ...
        # Control.aLinearX : None,
        # Control.aLinearY : None,
        # Control.aRotYaw  : None,
    }
    ...
    CollisionVol = {
        Frames.waist_yaw_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.waist_roll_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.waist_pitch_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.left_shoulder_pitch_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.left_shoulder_roll_joint: Geometry(
            type="sphere", radius=0.06, color=VizColor.collision_volume
        ),
        Frames.left_shoulder_yaw_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.left_elbow_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.left_wrist_roll_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.left_wrist_pitch_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.left_wrist_yaw_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.right_shoulder_pitch_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.right_shoulder_roll_joint: Geometry(
            type="sphere", radius=0.06, color=VizColor.collision_volume
        ),
        Frames.right_shoulder_yaw_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.right_elbow_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.right_wrist_roll_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.right_wrist_pitch_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
        Frames.right_wrist_yaw_joint: Geometry(
            type="sphere", radius=0.05, color=VizColor.collision_volume
        ),
