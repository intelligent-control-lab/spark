...
from spark_utils import Geometry, Volume, compute_pairwise_dist, compute_pairwise_info_parallel
import numpy as np
...
class BasicCollisionSafetyIndex(BaseSafetyIndex):
    ...
    def phi(self,
        ...
        for i in range(self.num_robot_vol):
    ...
    def get_vol_info(self, x, task_info):
        ...
        obstacle_vol_geom         = task_info["obstacle"]["geom"]
        obstacle_vol_velocity     = task_info["obstacle"]["velocity"]
        ...
        for i in range(self.num_robot_vol):
            frame_id = self.robot_kinematics.frame_dict[self.collision_vol_frame_ids[i].name]
            jacobian = self.robot_kinematics.compute_frame_jacobian(q, frame_id)[:3, :]
            robot_collision_vol.append(Volume(
                frame = trans_world2base @ robot_vol_frames[i, :, :],
                geometry  = self.collision_vol_geom[i],
        ...
        for i in range(self.num_obstacle_vol):
            obstacle_collision_vol.append(Volume(
                ...
                velocity = obstacle_vol_velocity[i]
            ))
    ...
    def compute_pairwise_info(self, volume_list_1, volume_list_2):
        ...
        """
        return compute_pairwise_info_parallel(
