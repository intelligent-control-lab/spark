...
from spark_safe.safe_algo.value_based.ssa.basic_safe_set_algorithm import BasicSafeSetAlgorithm
...
class RelaxedSafeSetAlgorithm(BasicSafeSetAlgorithm):
    ...
    def __init__(self, **kwargs):
        ...
        print("Initializing RelaxedSafeSetAlgorithm")
        # default values
        self.eta_default = kwargs["eta_ssa"]
    ...
    def safe_control(self,
        ...
        if (phi[self.safety_index.phi_mask>0].max() > 0):
            ...
            u_safe, violation = self.qp_solver(
                ...
                Q_u = Q_u,
                Lg = Lg,
                Lf = Lf.reshape(-1,1) + self.eta_fn(phi).reshape(-1,1)
            )
        else:
            trigger_safe = False
            u_safe = u_ref
