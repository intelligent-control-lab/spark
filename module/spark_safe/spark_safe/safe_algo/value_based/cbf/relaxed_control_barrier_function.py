...
from spark_safe.safe_algo.value_based.cbf.basic_control_barrier_function import BasicControlBarrierFunction

class RelaxedControlBarrierFunction(BasicControlBarrierFunction):
    '''
        Relaxed-CBF
        
        CBF with multiple phi_dot <= -lambda * phi constraints and slack variables for each constraint
        Slack variables are penalized quadratically in the QP solver with manual weights
    '''

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        print("Initializing RelaxedControlBarrierFunction")
        # default values
        self.lambda_default = kwargs["lambda_cbf"]
