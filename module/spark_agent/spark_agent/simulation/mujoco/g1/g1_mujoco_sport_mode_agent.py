...
class G1MujocoSportModeAgent(MujocoAgent):
    """
    ...
    def _init_sport_mode(self):
        ...
        self.policy_path = os.path.join(SPARK_AGENT_ROOT, "spark_agent/simulation/mujoco/g1/motion.pt")
    ...
    def reset(self) -> None:
        ...
        # Adjust default position for sport mode
    ...
    def get_feedback(self) -> None:
        ...
        # Update DoF velocity feedback for sport mode
        for dof in self.robot_cfg.DoFs:
            if dof.name == "LinearX":
    ...
    def _set_dof_pos(self, dof_pos: np.ndarray) -> None:
        ...
        # If sport_mode is enabled, directly set qpos
        qpos = dof_pos
    ...
    def _send_control_sim_dynamics(self, command, **kwargs):
        ...
        # Apply control in either sport mode 
