...
class LRMate200iD3fMujocoSingleAgent(MujocoAgent):
    """
    A Mujoco-based agent for the LRMate200iD 3F robot, extending MujocoAgent.
    ...
    def _mujoco_step(self):
        ...
        if self.enable_camera:
            ...
            # img_main, depth_main = self.render_camera(self.main_cam)
            # img_right, depth_right = self.render_camera(self.right_cam)  # dual arm only
            # img_left, depth_left = self.render_camera(self.left_cam)    # dual arm only
    ...
    def _send_control_sim_dynamics(self, command, **kwargs):
        ...
        gripper_control = action_info.get("gripper_control", [0,0,0,0])
        gripper_icf = action_info.get("gripper_icf", False)

        # Apply PD control to the robot's actuators
        for _ in range(self.control_decimation):
            ctrl = np.zeros(len(self.data.ctrl))
            ctrl[:6] = self.target_pos[:6]
            
            #complex mode
            ctrl[6:18] = self.controlGripperRegister(
                POA = gripper_control[0],
                POB = gripper_control[1],
                POC = gripper_control[2],
                POS = gripper_control[3],
                ICF = gripper_icf
            )
    ...
    def _send_control_modeled_dynamics(self, command, **kwargs):
        ...
        self.data.qpos[6:18]  = self.controlGripperRegister(
            POA = gripper_control[0],
            POB = gripper_control[1],
            POC = gripper_control[2],
            POS = gripper_control[3],
    ...
    def controlGripperRegister(self, POA = 0, POB = 0, POC = 0, POS = 137, ICF = False):
        ...
        # deg2rad
        commandGripper = np.deg2rad(commandGripper)
