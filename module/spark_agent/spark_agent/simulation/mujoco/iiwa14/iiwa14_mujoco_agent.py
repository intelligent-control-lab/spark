...
class IIWA14MujocoFixedBaseAgent(MujocoAgent):
    """
    ...
    def __init__(self, robot_cfg: RobotConfig, **kwargs) -> None:
        ...
        if self.enable_camera:
            ...
            mujoco.mjv_defaultCamera(self.right_cam)

            # Position each camera
            self.fixed_cam.lookat[:] = [0.1, 0.0, 1.0]
            ...
            self.right_cam.fixedcamid = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_CAMERA, "cam")

            self.right_cam.type = mujoco.mjtCamera.mjCAMERA_FIXED
            self.right_cam.fixedcamid = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_CAMERA, "right/cam")

            self.left_cam.type = mujoco.mjtCamera.mjCAMERA_FIXED
            self.left_cam.fixedcamid = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_CAMERA, "left/cam")
        ...
        self.kps = np.zeros(self.num_dof)
        self.kds = np.zeros(self.num_dof)
    ...
    def _send_control_sim_dynamics(self, command, **kwargs):
        ...
        for _ in range(self.control_decimation):
            ...
            ctrl[:7] = self.target_pos[:7]  # Apply control to the first 7 DoFs (right arm)
            # ctrl[8:15] = self.target_pos[7:]  # Apply control to the next
            
            # ctrl[7] = 255 if right_gripper_goal else 0  # Left gripper control
            # ctrl[15] = 255 if left_gripper_goal else 0  # Right gripper control
            
            self.data.ctrl[:7]= ctrl[:7]
            self.counter += 1
            # self._pick_and_place_objects(left_gripper_goal, right_gripper_goal)
            mujoco.mj_step(self.model, self.data)
            # self._mujoco_step()

    def _pick_and_place_objects(self, left_gripper_goal, right_gripper_goal):
        """
        Handles the picking and placing of objects based on the gripper states.
        """
        # If the left gripper turn from open to close, pick the object that is closes to it
        if left_gripper_goal and not self.left_gripper_picking:
            # Find the closest object to the left gripper
            closest_object = None
            min_distance = float('inf')
            for b in range(1, self.model.nbody):
                name = mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_BODY, b) or f"body_{b}"
                object_type = name.split("_")[0]  # Use the last part of the name
                if object_type in self.pickable_objects:
                    pos = self.data.xpos[b][:3]
                    distance = np.linalg.norm(pos - self.data.body("left/link7").xpos[:3])
                    if distance < min_distance:
                        min_distance = distance
                        closest_object = name
            
            if closest_object:
                self.left_gripper_picking = closest_object
        elif not left_gripper_goal and self.left_gripper_picking:
            self.left_gripper_picking = None
        
        if self.left_gripper_picking:
            # Set the position of the object to be picked by the left gripper
            self._set_object_pos(self.left_gripper_picking, -1)
            
        # If the right gripper turn from open to close, pick the object that is closes to it
        if right_gripper_goal and not self.right_gripper_picking:
            # Find the closest object to the right gripper
            closest_object = None
            min_distance = float('inf')
            for b in range(1, self.model.nbody):
                name = mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_BODY, b) or f"body_{b}"
                ...
                if object_type in self.pickable_objects:
                    pos = self.data.xpos[b][:3]
                    distance = np.linalg.norm(pos - self.data.body("right/link7").xpos[:3])
                    if distance < min_distance:
                        min_distance = distance
                        closest_object = name
            
            if closest_object:
                self.right_gripper_picking = closest_object
        elif not right_gripper_goal and self.right_gripper_picking:
            self.right_gripper_picking = None
        
        if self.right_gripper_picking:
            # Set the position of the object to be picked by the right gripper
            self._set_object_pos(self.right_gripper_picking, 1)
                        
                    

    def _set_object_pos(self, object_body, object_state):
        """
        object_state: -1 -> attach to left EE, +1 -> right EE, 0 -> do nothing
        object_body:  body name of the object that has a FREE joint (e.g., "wooden-block_0")
        """
        
        if object_state == 0:
            return

        # --- Pick the EE and its desired local offset (in EE frame) ---
        left_ee_name  = "left/link7"
        right_ee_name = "right/link7"

        if object_state == -1:
            ee_name = left_ee_name
            offset_local = np.array([0.0, 0.0, 0.18])     # example: along EE +Z
        else:
            ee_name = right_ee_name
            offset_local = np.array([0.0, 0.0, 0.18])     # example: along EE +X

        # --- EE pose in world ---
        ee_pos = self.data.body(ee_name).xpos[:3].copy()
        ee_R   = self.data.body(ee_name).xmat.copy().reshape(3, 3)  # world_from_ee rotation

        # Position at local offset (in world frame)
        target_pos = ee_pos + ee_R @ offset_local

        # Orientation: same as EE (or add a local rotation if you want)
        # Using SciPy for clarity:
        # (If you want a fixed rotation offset R_off_local, do: target_R = ee_R @ R_off_local)
        target_R = ee_R
        scipy_quat_xyzw = R.from_matrix(target_R).as_quat()  # [x,y,z,w]
        target_xquat = np.array([scipy_quat_xyzw[3], *scipy_quat_xyzw[:3]])  # -> [w,x,y,z] for MuJoCo

        # --- Resolve object FREE joint via the body ---
        bid = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, object_body)
        if bid < 0:
            raise ValueError(f"Body '{object_body}' not found.")

        jadr = self.model.body_jntadr[bid]
        jnum = self.model.body_jntnum[bid]
        if jnum == 0:
            raise ValueError(f"Body '{object_body}' has no joints.")
        jid = jadr
        if self.model.jnt_type[jid] != mujoco.mjtJoint.mjJNT_FREE:
            raise ValueError(f"Body '{object_body}' first joint is not FREE (type={self.model.jnt_type[jid]}).")

        qpos_adr = self.model.jnt_qposadr[jid]  # 7 dof: [px,py,pz,qw,qx,qy,qz]
        qvel_adr = self.model.jnt_dofadr[jid]   # 6 dof

        # --- Write pose (and zero velocity so it sticks) ---
        self.data.qpos[qpos_adr:qpos_adr+3] = target_pos
        self.data.qpos[qpos_adr+3:qpos_adr+7] = target_xquat
