...
class BaseAgent(ABC):
    ...
    def is_running(self) -> bool:
        """Return whether the agent backend can continue stepping.

        Hardware agents and non-interactive backends run until their pipeline
        stops them. Simulation viewers override this hook so closing a native
        window also terminates the pipeline instead of leaving a hidden
        simulator process alive.
        """
        return True
