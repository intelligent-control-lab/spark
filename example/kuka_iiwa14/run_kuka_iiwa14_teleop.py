...
from spark_pipeline import KukaIIWA14SingleArmTeleopPipelineConfig as PipelineConfig
...
def config_task_module(cfg: PipelineConfig, **kwargs):
    ...
    # The KUKA default joint pose already places the Robotiq tool frame at an
    # identity-orientation, gripper-down Cartesian home. Keep that explicit
    # pose consistent across MuJoCo and Isaac instead of deriving a potentially
    # different goal from each simulator's first feedback sample.
...
def run(**kwargs):
    ...
    if int(kwargs.get("num_envs", 1)) != 1:
        raise ValueError("KUKA safe teleoperation supports exactly one environment")
    robot_cfg_name = kwargs.get("robot_cfg")
    if not robot_cfg_name:
        raise ValueError("robot_cfg must select a KUKA iiwa 14 configuration")
    kwargs = dict(kwargs)
...
def _parse_cli_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Run single-environment KUKA iiwa 14 safe teleoperation."
    )
    ...
    parser.add_argument(
        ...
        default="KukaIIWA14SingleArmDynamic1CollisionConfig",
    )
