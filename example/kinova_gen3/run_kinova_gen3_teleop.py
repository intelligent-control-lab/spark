...
from spark_pipeline import KinovaGen3SingleArmTeleopPipelineConfig as PipelineConfig
...
def config_task_module(cfg: PipelineConfig, **kwargs):
    ...
    # Retain the original explicit, identity-orientation teleop homes instead
    # of interpreting a zero keyboard offset as "hold the reset pose". The
    # Kinova flange rotates the gripper so identity points it downward.
    ...
    if cfg.env.task.use_dual_arm:
        cfg.env.task.goal_left_init = [0.4, 0.3, 0.5]
        cfg.env.task.goal_right_init = [0.4, -0.3, 0.5]
    else:
        cfg.env.task.goal_right_init = [0.4, 0.0, 0.3]
    return cfg
...
def config_policy_module(cfg: PipelineConfig, **kwargs):
    ...
    # cfg.policy.nominal_controller.class_name = "TrajOptPolicy"
    return cfg
...
def run(**kwargs):
    ...
    if int(kwargs.get("num_envs", 1)) != 1:
        raise ValueError("Kinova safe teleoperation supports exactly one environment")
    robot_cfg_name = kwargs.get("robot_cfg")
    if not robot_cfg_name:
        raise ValueError("robot_cfg must select a Kinova Gen3 configuration")
    kwargs = dict(kwargs)
...
def _parse_cli_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Run single-environment Kinova Gen3 safe teleoperation."
    )
    ...
    parser.add_argument(
        ...
        default="KinovaGen3SingleArmDynamic1CollisionConfig",
    )
