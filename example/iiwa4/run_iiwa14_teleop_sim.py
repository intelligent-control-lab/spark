...
from spark_pipeline import IIWA142fTeleopPipelineConfig as PipelineConfig
...
def config_task_module(cfg: PipelineConfig, **kwargs):
    ...
    if "Single" not in cfg.robot.cfg.class_name:
...
def config_policy_module(cfg: PipelineConfig, **kwargs):
    ...
    # cfg.algo.policy.class_name = "TrajTrackingPolicy"
    return cfg
...
def config_safety_module(cfg: PipelineConfig, **kwargs):
    ...
    if cfg.algo.safe_controller.safe_algo.class_name != "ByPassSafeControl":
        if "Single" in cfg.robot.cfg.class_name:
            cfg.algo.safe_controller.safe_algo.control_weight = cfg.algo.safe_controller.safe_algo.control_weight[:7]
...
if __name__ == "__main__":
    ROBOT_CFG_LIST = [
        "IIWA14SingleDynamic1Config ",
        "IIWA14SingleDynamic1CollisionConfig ",
        "IIWA14DualDynamic1Config",
    ]

    AGENT_CFG_LIST = [
        "IIWA14MujocoFixedBaseAgent",
    ]
    
    run(robot_cfg = "IIWA14SingleDynamic1CollisionConfig",
        agent_cfg = "IIWA14MujocoFixedBaseAgent",
        safe_algo = "rssa",
        safety_index = "si1")